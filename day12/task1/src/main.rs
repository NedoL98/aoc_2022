        ...
        for line in lines {
            if let Ok(ip) = line {
                let tokens: Vec<&str> = 
                    ...
                    .filter(|s| !s.is_empty())
                    .collect();
                if let Some(pos) = tokens.iter().position(|&s| s == "S") {
                    input.start = (input.grid.len(), pos);
                }
                if let Some(pos) = tokens.iter().position(|&s| s == "E") {
                    input.finish = (input.grid.len(), pos);
                }
...
fn main() {
    ...
    while !queue.is_empty() {
        ...
        if v == input.finish {
            println!("{}", distances[v.0][v.1].unwrap());
            return;
        }
        ...
        for dx in vec![-1, 0, 1] {
            for dy in vec![-1, 0, 1] {
                ...
                if v.0 as i32 + dx >= 0 
                    ...
                    if input.grid[v.0][v.1] + 1 >= input.grid[new_pos.0][new_pos.1] 
